---
title: rpi-seism Daemon
description: Multi-threaded Python application for seismic data acquisition and processing.
sidebar:
  label: Overview
  order: 1
---

import { Card, CardGrid, Aside, Steps, Tabs, TabItem } from '@astrojs/starlight/components';

The **rpi-seism daemon** is the central nervous system of the project. It is a high-performance, multi-threaded Python application designed to acquire, process, store, and broadcast triaxial seismic data in real-time.

<Aside type="tip">
  Optimized for **Python 3.13+** and tested on Raspberry Pi OS (Debian Bookworm).
</Aside>

## System Architecture

The daemon acts as the bridge between raw hardware signals and high-level data analysis tools. It manages the high-speed RS-422 serial stream from the MCU and distributes it to specialized processing threads.



### Hardware Interface
- **Sensor:** 3x GD-4.5 Geophones (Triaxial)
- **Signal Chain:** LT1167 InstAmp → ADS1256 (24-bit ADC)
- **Transport:** RS-422 @ 250 kbps via MAX490E
- **Physical Layer:** Cat5/6 cabling for long-distance noise immunity

---

## Key Features

<CardGrid>
  <Card title="Real-time Acquisition" icon="rocket">
    Continuous 100 SPS sampling with CRC-32 checksum validation for every packet.
  </Card>
  <Card title="Standardized Storage" icon="document">
    Writes to **SDS (SeisComP Data Structure)** in MiniSEED format with automatic midnight splits.
  </Card>
  <Card title="Live Streaming" icon="external-link">
    Websocket server (ws://8765) broadcasting 4× decimated (25 Hz) filtered data.
  </Card>
  <Card title="Smart Alerts" icon="notification">
    Integrated STA/LTA trigger engine with Apprise support (Telegram, Email, etc.).
  </Card>
</CardGrid>

---

## Quick Start

<Steps>
1.  **Installation**
    Clone the repo and sync dependencies using `uv`:
    ```bash
    git clone https://github.com/rpi-seism/rpi-seism-daemon.git
    cd rpi-seism-daemon
    uv sync
    ```

2.  **Initial Configuration**
    Running the daemon for the first time generates a default `config.yml`:
    ```bash
    mkdir -p data
    uv run rpi-seism
    nano data/config.yml
    ```

3.  **Deployment**
    ```bash
    uv run rpi-seism
    ```
</Steps>

---

## Threading Model

The daemon utilizes five concurrent threads to ensure no data loss during heavy I/O operations:

| Thread | Responsibility | Output |
| :--- | :--- | :--- |
| **Reader** | Serial parsing & CRC validation | Internal Queues |
| **MSeedWriter** | SDS Archival (MiniSEED) | `/data/archive/` |
| **WSSender** | Decimation & Live Broadcast | `ws://port:8765` |
| **Trigger** | STA/LTA Event Detection | Earthquake Event |
| **Notifier** | Alert Delivery (Apprise) | Telegram/Email |



---

## Technical Specifications

### Data Standards
- **Format:** MiniSEED (FDSN compliant).
- **Metadata:** Auto-generated **StationXML** with pole-zero instrument response.
- **Structure:** SeisComP Data Structure (SDS) `YEAR/NET/STA/CHAN.D/`.

---

## Next Steps

<CardGrid>
  <linkCard title="Hardware Protocol" href="/daemon/serial-protocol" description="Deep dive into the 16-byte RS-422 packet format." />
  <linkCard title="Configuration Guide" href="/daemon/configuration" description="How to tune STA/LTA thresholds and ADC gain." />
</CardGrid>
